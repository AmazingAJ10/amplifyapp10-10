...
import { getSubscriptionApp } from "../graphql/queries";
import { updateSubscriptionApp } from "../graphql/mutations";
export default function SubscriptionAppUpdateForm(props) {
  const {
    id: idProp,
    subscriptionApp: subscriptionAppModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = subscriptionAppRecord
      ? { ...initialValues, ...subscriptionAppRecord }
      : initialValues;
    setSubscriptionName(cleanValues.SubscriptionName);
    setNextPayment(cleanValues.NextPayment);
    setCost(cleanValues.Cost);
    setLogo(cleanValues.Logo);
    setErrors({});
  };
  const [subscriptionAppRecord, setSubscriptionAppRecord] = React.useState(
    subscriptionAppModelProp
  );
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? (
            await API.graphql({
              query: getSubscriptionApp.replaceAll("__typename", ""),
              variables: { id: idProp },
            })
          )?.data?.getSubscriptionApp
        : subscriptionAppModelProp;
      setSubscriptionAppRecord(record);
    };
    queryData();
  }, [idProp, subscriptionAppModelProp]);
  React.useEffect(resetStateValues, [subscriptionAppRecord]);
  const validations = {
    SubscriptionName: [],
    NextPayment: [],
    Cost: [],
    ...
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await API.graphql({
            query: updateSubscriptionApp.replaceAll("__typename", ""),
            variables: {
              input: {
                id: subscriptionAppRecord.id,
                ...modelFields,
              },
            },
          });
      ...
      {...getOverrideProps(overrides, "SubscriptionAppUpdateForm")}
      {...rest}
    >
      <TextField
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || subscriptionAppModelProp)}
